...
required_cols <- c("STUDYID", "USUBJID", "TRT01A", "PARAMCD", "AVAL", "CNSR", "EVNTDESC", "ANL01FL")
...
cox_ties <- as.character(spec$analysis$cox_ties)
...
if (!identical(tolower(cox_ties), "efron")) stop("v0.17 Cox tie method must be efron")

d <- d[d$PARAMCD == expected_paramcd & d$ANL01FL == "Y" & d$TRT01A %in% expected_arms, , drop = FALSE]
...
d <- d[order(d$TRT01A, d$USUBJID), , drop = FALSE]
row.names(d) <- NULL
...
add_check("TTE analysis has all three treatment arms", identical(levels(droplevels(d$TRT01A)), expected_arms), paste(levels(droplevels(d$TRT01A)), collapse = ", "))
add_check("TTE analysis has one row per subject", !anyDuplicated(d$USUBJID), paste0("duplicate subjects=", sum(duplicated(d$USUBJID))))
...
km_fit <- survfit(Surv(AVAL, status) ~ TRT01A, data = d, conf.type = "log-log")
km_sum <- summary(km_fit, times = km_times, extend = TRUE)
km <- data.frame(
  TRT01A = sub("^TRT01A=", "", as.character(km_sum$strata)),
...
km$TRT01A <- factor(km$TRT01A, levels = expected_arms)
km <- km[order(km$TRT01A, km$time_days), , drop = FALSE]
km$TRT01A <- as.character(km$TRT01A)
row.names(km) <- NULL
...
median_rows <- data.frame(
  TRT01A = sub("^TRT01A=", "", rownames(km_table)),
...
median_rows$TRT01A <- factor(median_rows$TRT01A, levels = expected_arms)
median_rows <- median_rows[order(median_rows$TRT01A), , drop = FALSE]
median_rows$TRT01A <- as.character(median_rows$TRT01A)
...
pairwise_rows <- lapply(active_arms, function(active_arm) {
  dd <- d[d$TRT01A %in% c("Placebo", active_arm), , drop = FALSE]
  dd$TRT01A <- relevel(droplevels(dd$TRT01A), ref = "Placebo")

  logrank <- survdiff(Surv(AVAL, status) ~ TRT01A, data = dd, rho = 0)
  logrank_chisq <- as.numeric(logrank$chisq)
  logrank_p <- stats::pchisq(logrank_chisq, df = 1, lower.tail = FALSE)

  cox_fit <- coxph(
    Surv(AVAL, status) ~ TRT01A,
    data = dd,
    ties = cox_ties,
    x = TRUE,
    y = TRUE,
...
add_check("KM probabilities and confidence limits are finite and bounded", all(is.finite(as.matrix(km[, c("retention_probability", "standard_error", "ci95_lower", "ci95_upper")])) & km$retention_probability >= 0 & km$retention_probability <= 1 & km$ci95_lower >= 0 & km$ci95_upper <= 1), paste0("rows=", nrow(km)))

monotone_ok <- all(vapply(split(km, km$TRT01A), function(z) all(diff(z$retention_probability) <= 1e-12), logical(1)))
add_check("KM retention probabilities are non-increasing over time", monotone_ok, paste0("arms=", length(unique(km$TRT01A))))
add_check("Pairwise table has exactly two active-versus-placebo comparisons", nrow(pairwise) == 2, paste0("rows=", nrow(pairwise)))
add_check("Cox hazard ratios and confidence intervals are finite and positive", all(is.finite(as.matrix(pairwise[, c("hazard_ratio", "ci95_lower", "ci95_upper")])) & pairwise$hazard_ratio > 0 & pairwise$ci95_lower > 0 & pairwise$ci95_upper > 0 & pairwise$ci95_lower <= pairwise$hazard_ratio & pairwise$hazard_ratio <= pairwise$ci95_upper), paste0("rows=", nrow(pairwise)))
add_check("Cox and log-rank p-values are valid", all(is.finite(as.matrix(pairwise[, c("cox_p_value", "logrank_p_value")])) & pairwise$cox_p_value >= 0 & pairwise$cox_p_value <= 1 & pairwise$logrank_p_value >= 0 & pairwise$logrank_p_value <= 1), paste0("rows=", nrow(pairwise)))
add_check("Proportional-hazards diagnostic p-values are available", all(is.finite(pairwise$ph_test_p_value) & pairwise$ph_test_p_value >= 0 & pairwise$ph_test_p_value <= 1), paste0("rows=", nrow(pairwise)))
...
jsonlite::write_json(metrics, file.path(out_dir, "tte_retention_survival_metrics.json"), pretty = TRUE, auto_unbox = TRUE, na = "null")

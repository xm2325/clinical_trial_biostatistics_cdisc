...
Source: official CDISC pilot `QS` Dataset-JSON, filtered to SDTM `QSTESTCD=CIBIC` and linked to ADSL-style. The analysis parameter is `CIBICVAL`, matching the public reference ADaM.
...
| AWTARGET | public reference metadata | 56 / 112 / 168 days |
| AWLO / AWHI | public reference metadata | 2–84 / 85–140 / 141+ |
...
Source: official CDISC pilot `QS` Dataset-JSON, filtered to `QSTESTCD=ACTOT`. The official SDTM Define-XML identifies ACTOT as a derived total score for the Alzheimer's Disease Assessment Scale.

| Variable | Source | Derivation |
|---|---|---|
| STUDYID / USUBJID | QS | copied key |
| TRT01A | ADSL-style | actual treatment |
| PARAMCD | QS.QSTESTCD | `ACTOT` |
| PARAM | QS.QSTEST | source total-score label |
| AVISIT / AVISITN | QS.VISIT / VISITNUM | source visit |
| ADY / ADT | QS.QSDY / QSDTC | numeric day and parsed date |
| AVAL | QS.QSSTRESN | numeric standard total score |
| BASE | QS.QSBLFL | subject baseline value from the baseline-flagged ACTOT record |
...
Observed Week 24 uses an observed `WEEK 24` ACTOT record. LOCF sensitivity uses the latest numeric post-baseline ACTOT observation on or before day 168. Each analysis dataset contains one row per subject with treatment, BASE, Week 24/sensitivity AVAL, CHG, source day and `DTYPE`.

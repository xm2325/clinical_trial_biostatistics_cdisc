...
def test_cibic_windowing_and_locf():
    ...
    qs = pd.DataFrame([
        {"STUDYID":"S","USUBJID":"1","QSSEQ":6001,"QSTESTCD":"CIBIC","QSSTRESN":3,"QSDY":29,"QSDTC":"2020-01-29","VISIT":"WEEK 4","VISITNUM":5},
        {"STUDYID":"S","USUBJID":"1","QSSEQ":6002,"QSTESTCD":"CIBIC","QSSTRESN":5,"QSDY":198,"QSDTC":"2020-07-17","VISIT":"RETRIEVAL","VISITNUM":201},
    ])
    ...
    assert set(out["PARAMCD"]) == {"CIBICVAL"}


def test_actot_derivation_and_ancova():
    rows = []
    seq = 1
    pairs = [(30,28),(31,25),(32,24),(29,30),(33,26),(34,25)]
    for uid, (base, wk24) in enumerate(pairs, start=1):
        rows.extend([
            {"STUDYID":"S","USUBJID":str(uid),"QSSEQ":seq,"QSTESTCD":"ACTOT","QSTEST":"ADAS TOTAL SCORE","QSSTRESN":base,"QSBLFL":"Y","QSDY":1,"QSDTC":"2020-01-01","VISIT":"BASELINE","VISITNUM":3},
            {"STUDYID":"S","USUBJID":str(uid),"QSSEQ":seq+1,"QSTESTCD":"ACTOT","QSTEST":"ADAS TOTAL SCORE","QSSTRESN":wk24,"QSBLFL":"","QSDY":168,"QSDTC":"2020-06-17","VISIT":"WEEK 24","VISITNUM":12},
        ])
        seq += 10
    adqs = derive_actot_adqs_style(pd.DataFrame(rows), _adsl())
    assert int(adqs["ABLFL"].eq("Y").sum()) == 6
    assert set(adqs["PARAMCD"]) == {"ACTOT"}
    post = adqs.loc[adqs["ABLFL"].ne("Y")]
    assert (post["CHG"] == post["AVAL"] - post["BASE"]).all()
    lsmeans, contrasts, subjects = actot_week24_ancova(adqs)

...
from .change_control_v016 import load_versioned_change_control as load_v016_change_control


GRAPH_EXTENSION = "change_impact_graph_v0_17_extension.json"
REQUEST_EXTENSION = "change_requests_v0_17_extension.json"


def _require_v017_versions(
    prior_version: str,
    ...
    if graph_base != prior_version or request_base != prior_version:
        raise ValueError(
            "v0.17 extensions must declare the exact merged v0.16 base version "
    ...
    if not graph_version or graph_version != request_version:
        raise ValueError(
            f"v0.17 extension version mismatch: graph={graph_version}; requests={request_version}"
        )
    if graph_version == prior_version:
        raise ValueError("v0.17 extension must advance beyond the merged v0.16 version")
...
def load_versioned_change_control(
    ...
    prior_graph, prior_requests, prior_paths, prior_version = load_v016_change_control(root)
    spec_dir = root / "spec"
    ...
    version = _require_v017_versions(
        prior_version,
        graph_extension,
        request_extension,
    )
    ...
    if graph.get("version") != version or requests.get("version") != version:
        raise ValueError("merged change-control version did not resolve to v0.17 extension version")

    paths = dict(prior_paths)
    paths["graph_extension_v017"] = graph_path
    paths["request_extension_v017"] = request_path
...
def run_change_impact_assessment(
    ...
    metrics = {
        ...
        "prior_extension_version": "0.16.0",
    ...
    lines = [
        ...
        f"(byte-preserved base {base_graph_version} + v0.15 + v0.16 + v0.17 controlled extensions)",
        "",
    ...
    lines.extend(
        [
            "",
            "v0.17 layers an exploratory retention time-to-event definition, ADTTE-style derivation, "
            "survival-analysis evidence and CR-011 over the validated v0.16 logical graph without "
            "rewriting earlier controlled specifications.",
            "",
            "The retention chain is intentionally separate from the ACTOT confirmatory multiplicity family. "
            "The gate derives required impacts from the merged dependency graph; every required downstream "
            "review item must be declared. Conservative extras are reported but do not fail the gate.",
        ]
    )
    return rows, metrics, "\n".join(lines) + "\n"

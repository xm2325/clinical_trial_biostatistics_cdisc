...
from .change_control_v020 import load_versioned_change_control as load_v020_change_control

GRAPH_EXTENSION = "change_impact_graph_v0_23_extension.json"
REQUEST_EXTENSION = "change_requests_v0_23_extension.json"


def _require_v023_versions(
    prior_version: str,
    ...
    if graph_base != prior_version or request_base != prior_version:
        raise ValueError(
            "v0.23 extensions must declare the exact merged v0.20 base version "
    ...
    if not graph_version or graph_version != request_version:
        raise ValueError(
            f"v0.23 extension version mismatch: graph={graph_version}; requests={request_version}"
        )
    if graph_version == prior_version:
        raise ValueError("v0.23 extension must advance beyond the merged v0.20 version")
...
def load_versioned_change_control(
    ...
    prior_graph, prior_requests, prior_paths, prior_version = load_v020_change_control(root)
    spec_dir = root / "spec"
    ...
    version = _require_v023_versions(prior_version, graph_extension, request_extension)
    ...
    if graph.get("version") != version or requests.get("version") != version:
        raise ValueError("merged change-control version did not resolve to v0.23 extension version")
    paths = dict(prior_paths)
    paths["graph_extension_v023"] = graph_path
    paths["request_extension_v023"] = request_path
...
def run_change_impact_assessment(
    ...
    metrics = {
        ...
        "prior_extension_version": "0.20.0",
    ...
    lines = [
        ...
        "",
        f"Change-control specification version: **{version}** (byte-preserved base {base_graph_version} + v0.15 + v0.16 + v0.17 + v0.18 + v0.19 + v0.20 + v0.23 controlled extensions)",
        "",
        ...
        f"Extra declared resources: **{extra_declared}**",
    ...
    lines.extend(
        [
            "",
            "v0.23 adds CR-015 because the treatment-assignment source materially changes T20/T21/T22 missing-data sensitivity populations. The current observed primary MMRM result is not recalculated by CR-015 because all 12 planned/actual mismatch subjects have zero observed Week 8/16/24 ACTOT records; instead a blocking guard makes any future mismatch entry into that MMRM explicit.",
            "",
            "CR-015 preserves actual-treatment provenance while requiring planned randomised assignment for the randomised efficacy sensitivity target and 170-subject active-versus-placebo pairwise MI populations.",
        ]
    )
    return rows, metrics, "\n".join(lines) + "\n"

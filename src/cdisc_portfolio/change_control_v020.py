...
from .change_control_v019 import load_versioned_change_control as load_v019_change_control

GRAPH_EXTENSION = "change_impact_graph_v0_20_extension.json"
REQUEST_EXTENSION = "change_requests_v0_20_extension.json"


def _require_v020_versions(
    prior_version: str,
    ...
    if graph_base != prior_version or request_base != prior_version:
        raise ValueError(
            "v0.20 extensions must declare the exact merged v0.19 base version "
    ...
    if not graph_version or graph_version != request_version:
        raise ValueError(
            f"v0.20 extension version mismatch: graph={graph_version}; requests={request_version}"
        )
    if graph_version == prior_version:
        raise ValueError("v0.20 extension must advance beyond the merged v0.19 version")
...
def load_versioned_change_control(
    ...
    prior_graph, prior_requests, prior_paths, prior_version = load_v019_change_control(root)
    spec_dir = root / "spec"
    ...
    version = _require_v020_versions(prior_version, graph_extension, request_extension)
    ...
    if graph.get("version") != version or requests.get("version") != version:
        raise ValueError("merged change-control version did not resolve to v0.20 extension version")
    paths = dict(prior_paths)
    paths["graph_extension_v020"] = graph_path
    paths["request_extension_v020"] = request_path
...
def run_change_impact_assessment(
    ...
    metrics = {
        ...
        "prior_extension_version": "0.19.0",
    ...
    lines = [
        ...
        "",
        f"Change-control specification version: **{version}** (byte-preserved base {base_graph_version} + v0.15 + v0.16 + v0.17 + v0.18 + v0.19 + v0.20 controlled extensions)",
        "",
    ...
    lines.extend(
        [
            "",
            "v0.20 adds a study-statistician analysis-readiness and evidence-closure layer over the validated v0.19 analysis package without changing the statistical TLF registry.",
            "",
            "CR-014 intentionally has no impacted TLFs. It controls the analysis cutoff, blinded aggregate readiness checks, retained-issue disposition and portfolio-only readiness/closure definitions; statistical estimands, models and TLF results remain unchanged.",
        ]
    )
    return rows, metrics, "\n".join(lines) + "\n"

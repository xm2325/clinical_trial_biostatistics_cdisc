...
from .change_control_v017 import load_versioned_change_control as load_v017_change_control

GRAPH_EXTENSION = "change_impact_graph_v0_18_extension.json"
REQUEST_EXTENSION = "change_requests_v0_18_extension.json"


def _require_v018_versions(prior_version: str, graph_extension: dict[str, Any], request_extension: dict[str, Any]) -> str:
    ...
    if graph_base != prior_version or request_base != prior_version:
        raise ValueError(
            "v0.18 extensions must declare the exact merged v0.17 base version "
    ...
    if not graph_version or graph_version != request_version:
        raise ValueError(f"v0.18 extension version mismatch: graph={graph_version}; requests={request_version}")
    if graph_version == prior_version:
        raise ValueError("v0.18 extension must advance beyond the merged v0.17 version")
    return graph_version


def load_versioned_change_control(root: Path) -> tuple[dict[str, Any], dict[str, Any], dict[str, Path], str]:
    prior_graph, prior_requests, prior_paths, prior_version = load_v017_change_control(root)
    spec_dir = root / "spec"
    ...
    version = _require_v018_versions(prior_version, graph_extension, request_extension)
    ...
    if graph.get("version") != version or requests.get("version") != version:
        raise ValueError("merged change-control version did not resolve to v0.18 extension version")
    paths = dict(prior_paths)
    paths["graph_extension_v018"] = graph_path
    paths["request_extension_v018"] = request_path
...
def run_change_impact_assessment(root: Path) -> tuple[list[dict[str, Any]], dict[str, Any], str]:
        ...
        for category in IMPACT_CATEGORIES:
            ...
            for resource in resources:
                ...
                if required:
                    ...
                    extra_declared += 1
                status = "missing_required" if required and not declared else "extra_declared" if declared and not required else "matched"
                rows.append({
                    "change_id": assessment["change_id"], "title": assessment["title"], "category": category,
                    "resource": resource, "resolved_path": resolved_path, "required_by_graph": required,
                    "declared_for_review": declared, "resource_exists": exists, "status": status,
                })
    ...
    metrics = {
        ...
        "prior_extension_version": "0.17.0",
        ...
        "spec_sha256": {path.name: sha256_file(path) for key, path in paths.items() if key != "traceability"}
    ...
    lines = [
        ...
        f"Change-control specification version: **{version}** (byte-preserved base {base_graph_version} + v0.15 + v0.16 + v0.17 + v0.18 controlled extensions)", "",
    ...
    lines.extend([
        "",
        "v0.18 layers controlled analysis-metadata definition, executable variable/source lineage validation and a deterministic Define-XML-inspired export over the validated v0.17 graph.",
        "",
        "CR-012 intentionally has no impacted TLFs and does not alter the ACTOT confirmatory family or exploratory retention analysis. The metadata layer describes and validates existing generated analysis datasets; it does not claim formal ADaM or Define-XML conformance.",
    ])
    return rows, metrics, "\n".join(lines) + "\n"
